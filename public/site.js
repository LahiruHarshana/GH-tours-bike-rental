/* Generated from the approved single-file Lanka Loop website. */
(function () {
  ...
  const WHATSAPP_NUMBER="94770000000";
      ...
      const quickBook=document.getElementById("quickBook");if(quickBook){quickBook.addEventListener("submit",e=>{e.preventDefault();const d=new FormData(e.currentTarget),service=d.get("service"),start=d.get("start"),date=d.get("date"),travellers=d.get("travellers");openModal(`<strong>${service}</strong><br>Start: ${start}<br>Date: ${date}<br>${travellers}`,`Hello Lanka Loop! I would like a quote.\n\nService: ${service}\nPickup / start: ${start}\nDate: ${date}\nTravellers: ${travellers}`)})}
      document.getElementById("mainBookingForm").addEventListener("submit",e=>{e.preventDefault();const d=new FormData(e.currentTarget),active=document.querySelector(".booking-tab.active").dataset.tab;let html="",text="";if(active==="taxi"){html=`<strong>Airport taxi</strong><br>${d.get("taxiPickup")||"Pickup"} → ${d.get("taxiDestination")||"Destination"}<br>${d.get("taxiDate")||""} · ${d.get("taxiTime")||""}<br>${d.get("taxiPassengers")||""} · ${d.get("taxiVehicle")||""}`;text=`Hello Lanka Loop! I would like an airport taxi quote.\n\nPickup: ${d.get("taxiPickup")||""}\nDestination: ${d.get("taxiDestination")||""}\nDate: ${d.get("taxiDate")||""}\nTime / flight: ${d.get("taxiTime")||""}\nPassengers: ${d.get("taxiPassengers")||""}\nVehicle: ${d.get("taxiVehicle")||""}`}else if(active==="safari"){html=`<strong>Private safari</strong><br>${d.get("safariPark")||""}<br>${d.get("safariDate")||""} · ${d.get("safariSession")||""}<br>${d.get("safariGuests")||""} · Pickup: ${d.get("safariPickup")||"To confirm"}`;text=`Hello Lanka Loop! I would like a safari quote.\n\nPark: ${d.get("safariPark")||""}\nDate: ${d.get("safariDate")||""}\nSession: ${d.get("safariSession")||""}\nGuests: ${d.get("safariGuests")||""}\nPickup: ${d.get("safariPickup")||""}\nNotes: ${d.get("safariNotes")||""}`}else if(active==="bike"){html=`<strong>Bike rental</strong><br>${d.get("bikeType")||""} in ${d.get("bikeTown")||"Sri Lanka"}<br>${d.get("bikeStart")||""} → ${d.get("bikeEnd")||""}<br>${d.get("bikeCount")||""}`;text=`Hello Lanka Loop! I would like a bike rental quote.\n\nTown: ${d.get("bikeTown")||""}\nBike: ${d.get("bikeType")||""}\nStart: ${d.get("bikeStart")||""}\nEnd: ${d.get("bikeEnd")||""}\nCount: ${d.get("bikeCount")||""}\nNotes: ${d.get("bikeNotes")||""}`}else{html=`<strong>Private tour</strong><br>${d.get("tourDuration")||""} from ${d.get("tourStart")||"Sri Lanka"}<br>${d.get("tourDate")||""} · ${d.get("tourGuests")||""}<br>${d.get("tourInterests")||""}`;text=`Hello Lanka Loop! I would like a private tour quote.\n\nStart date: ${d.get("tourDate")||""}\nDuration: ${d.get("tourDuration")||""}\nGuests: ${d.get("tourGuests")||""}\nStart location: ${d.get("tourStart")||""}\nPlaces and interests: ${d.get("tourInterests")||""}`}openModal(html,text)});
